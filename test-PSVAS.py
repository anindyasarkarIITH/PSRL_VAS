...
def test(epoch, best_sr): 
    search_budget = 18 #random.randrange(12,19,3) 
    num_image = 0
    ...
    for batch_idx, (inputs, targets) in tqdm.tqdm(enumerate(testloader), total=len(testloader)):
        ...
        search_agent = c(agent)
        optimizer = optim.Adam(search_agent.parameters(), lr=0.0001) #1 foe 12,15 2 for 18,15 0.0002
        ...
        for step_ in range(search_budget): 
            ...
            logit, grid_prob_ = search_agent.forward(inputs, search_info, query_left) #, targets.cuda()
            #logit, grid_prob = search_agent.forward(inputs, search_info, query_left, mask_info) #, targets.cuda()
            ...
            for sample_id in range(int(inputs.shape[0])):
                ...
                if (int(reward_update[sample_id]) == 1):
                    #print ("1:", reward_update[sample_id])
                    act_target_label[sample_id, int(policy_sample[sample_id].data)] = 1
                else: 
                    #print ("2:", reward_update[sample_id])
...
#agent = utils.Model_search_Arch_diffbudVIS_()  #_
