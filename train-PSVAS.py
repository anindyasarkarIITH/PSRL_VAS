...
#from tensorboard_logger import configure, log_value /home/research/
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in tqdm.tqdm(enumerate(trainloader), total=len(trainloader)):
        ...
        for step_ in range(search_budget):
            ...
            logit, grid_prob = agent.forward(inputs, search_info, query_left) #, targets.cuda()
            ...
            ##mask grid out prob
            loss_cls = loss_static(grid_prob_net.float(), targets.float().cuda()) #grid_prob_out
        ...
        loss = loss_rl + (0.1) * loss_ce  #(0.0001) *
...
def test(epoch, best_sr): 
    ...
    if (search_budget == 12):
        ...
        if (recall> best_sr):
            ...
            torch.save(state,"/storage1/model_vas_xview99_sc_adapt_F")
...
agent = utils.Model_search_Arch_Adapt()  #_
...
#optimizer_cls = optim.Adam(agent.parameters(), lr=0.001)
